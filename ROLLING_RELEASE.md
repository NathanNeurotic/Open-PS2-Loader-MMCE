...
| `RIPTOPL-VARIANTS-*.zip` | Alternate build configs across the moving and pinned ps2dev flavours; the normalizer also adds one ready-made DualSense (`DUALSENSE=1`) loader for each available official flavour. Also carries the two RetroAchievements loaders per ps2dev flavour, `RIPTOPL-ra-pademu0*.ELF` and `RIPTOPL-ra-pademu1*.ELF` — **development builds, not a finished feature**: the in-game telemetry and the menu side that asks the PC about a game are both written, but none of it has been tested on hardware. See [docs/RETROACHIEVEMENTS.md](docs/RETROACHIEVEMENTS.md). |
