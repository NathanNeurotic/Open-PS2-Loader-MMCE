...
2. **`-qb` quickboot (revised after hardware evidence).** This audit originally said never to emit it after a RiptOPL menu session. Later USB testing proved that conclusion wrong for the direct no-reset BDM handoff, and revision 2692 iLink testing reproduced the same reset-boundary failure in every SDK flavour while Ember's keep-IOP path passed. RiptOPL now auto-emits `-qb` for **USB and iLink only**, where its bridge deliberately preserves the mounted BDM environment Neutrino needs to reach the image. Do not generalize that exception to unrelated backends without hardware evidence.
