...
// SDK header without C++ linkage guards (declares ps2sdk_get_iop_fd): wrap at the use site.
extern "C" {
#include <ps2sdkapi.h>
}
...
extern "C" void bdmInitDevicesData();
extern "C" int bdmUpdateDeviceData(item_list_t *itemList);
...
int bdmReadDriverName(int dir, char *driverName, int driverNameLength)
{
    ...
    result = fileXioIoctl(dir, USBMASS_IOCTL_GET_DRIVERNAME, (void *)"");
    if (result < 0)
...
void bdmLoadModules(void)
{
    ...
    ioPutSimpleAction(&bdmLoadBlockDeviceModules);
}
...
static int bdmNeedsUpdate(item_list_t *itemList)
{
    ...
    if (bdmShouldQueueModuleLoad() && lastModuleLoadGen != BdmGeneration) {
        ...
        if (ioPutSimpleAction(&bdmLoadBlockDeviceModules) == IO_OK)
            ...
            if (vmcSizeInMb > 0) {
                ...
                if (fd >= 0) {
                    ...
                    if (fileXioIoctl2(iop_fd, USBMASS_IOCTL_GET_LBA, NULL, 0, &startingLBA, sizeof(startingLBA)) == 0 && (startCluster = (unsigned int)fileXioIoctl(iop_fd, USBMASS_IOCTL_GET_CLUSTER, vmc_path)) != 0) {
                        ...
                        else if (fileXioIoctl(iop_fd, USBMASS_IOCTL_CHECK_CHAIN, (void *)"") == 1) {
