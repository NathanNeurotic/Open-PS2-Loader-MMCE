...
/* ---- RiptOPL parity-test stubs (#340, DO NOT MERGE) -----------------------------------------------
 * Everything above this line is upstream ps2homebrew/Open-PS2-Loader src/pad.c VERBATIM (master
 * 3e3f34e9). This build answers one question: does OFFICIAL's navigation/pad code, dropped into
 * RiptOPL's otherwise-unchanged menu environment (mmceman GameID arm included, per the tester's
 * config), scroll smoothly on the affected console? The fork additions the rest of the tree links
 * against are stubbed inert below: menu rumble (#172, opt-in feature), the transition edge-freeze,
 * and the Debug-HUD pad diagnostics (the HUD renders zeros in this build -- it is a FEEL-only
 * test by design).
 */
...
{
    if (out) {
        static const pad_diag_t zeroed = {0};
        *out = zeroed;
        out->analogCapable = -1;
    }
}
