...
static void unloadLwnbdSvr(void)
{
    ...
    // LOG macros are available via include/ioman.h
    // But LOG_INIT() was a macro in opl.c.
    // We assume LOG_INIT/LOG_ENABLE logic is handled in opl.c main or we need to replicate it?
    // In opl.c, LOG_INIT() was just `ee_sio_start` if EESIO_DEBUG.
    // And LOG_ENABLE was `ioPutRequest(..., &debugSetActive)`.
    // Since we are in a separate file, we might not have access to debugSetActive if it's not exposed.
    // debug.h has `int debugSetActive(void);`.
