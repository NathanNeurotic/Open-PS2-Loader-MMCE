...
/* RA: SMB server, gateway and the PC client running RetroAchievements;
   ...
   Kept under INGAME_UDP, unlike upstream, which raises it for every in-game
   build. Two extra entries are two extra entries in every game the default
   build ever launches, and a wider ARP table earns its keep only when raudp
   is actually resolving a third host. */
