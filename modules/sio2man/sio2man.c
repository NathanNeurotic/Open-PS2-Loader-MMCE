...
/* ---- RiptOPL local patch (issue #340): priority-ceiling transaction bracket -------------------
 * ps2sdk PR #709 removed this module's dedicated priority-24 worker thread; a transaction
 * (sio2_*_transfer_init .. sio2_transfer_reset) now runs every bus phase in the CALLING thread
 * while m_transfer_semaphore is held, and IOP kernel semaphores have no priority inheritance.
 * A low-priority holder (mcserv's RPC thread runs at 0x68) preempted by usbd (0x1E/0x24)
 * mid-transaction stalls freepad's priority-20 vblank poll past its window, which the EE sees
 * as padRead misses: menu inputs drop on real hardware under USB load (ps2sdk PR #898
 * acknowledges the failure class). Restore the old scheduling guarantee by boosting the
 * semaphore holder to the retired worker thread's priority for the life of the transaction.
 * While the holder BLOCKS (sio2_wait_for_intr) it yields the CPU exactly as before; only its
 * bus-phase code runs boosted, matching the pre-#709 design. Single holder by construction
 * (the semaphore), so one saved thid/prio pair suffices.
 */
#define SIO2MAN_XFER_PRIO 0x18

static int m_ceiling_thid;
static int m_ceiling_prio;

static void sio2_prio_ceiling_acquire(void)
{
	iop_thread_info_t info;
	int thid;

	thid = GetThreadId();
	if ( thid > 0 && ReferThreadStatus(thid, &info) == 0 && info.currentPriority > SIO2MAN_XFER_PRIO )
	{
		m_ceiling_thid = thid;
		m_ceiling_prio = info.currentPriority;
		ChangeThreadPriority(thid, SIO2MAN_XFER_PRIO);
	}
}
